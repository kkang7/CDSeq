...
function [estProp, estGEP, estT, logPosterior, cell_type_assignment, estProp_all,estGEP_all] = CDSeq(mydata, beta, alpha, T, N, gene_length, referenceGEP, poolsize)
%-------------------------------------------------------------------------
% inputs:
%-------------------------------------------------------------------------
...
if nargin>8
    error('CDSeq requires at most 8 input arguments')
end
...
if nT==1
    ...
    if nargin==5, estGEP = estGEP_read;end
    if nargin>5
%         if size(mydata,1)~=length(gene_length)
%             error('The number of genes in mydata has to be the same as that of in gene_length');
%         end
    ...
    if nargout>=5 && nargin==7,cell_type_assignment=ro;end
